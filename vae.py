...
#   This outsources uncertainty to auxiliary r.v. (see [1]: Sec 2.4)
...
# train_op = tf.train.AdamOptimizer(ADAM_RATE).minimize(-ELBO_estimate)
...
with tf.Session() as sess:
    ...
    for t in iters:
        ...
        ELBO, neg_KL, ll, mu, log_var, p = np.mean(ELBO), np.mean(neg_KL), np.mean(ll), np.mean(mu,0), np.mean(log_var,0), np.mean(p,0)
    ...
    titles = ["$\mathcal{L}(\phi,\\theta;x)$",
              ...
              "$\log(p_{\\theta}(x|z))$"]
    params = {"$\eta_{Adagrad}$":ADG_RATE, "$Activation$":"ReLU", "$Batch$ $Size$":M, "$MCE$ $Samples$":L, "$Latent$ $Dim$":J}
